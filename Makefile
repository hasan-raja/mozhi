.PHONY: api worker lint fmt typecheck test ci

# WSL: venv on native fs (~/.venvs/mozhi) + copy mode (no hardlinks on /mnt/f)
UV_LINK_MODE := copy
UV_PROJECT_ENVIRONMENT := $(HOME)/.venvs/mozhi
export UV_LINK_MODE UV_PROJECT_ENVIRONMENT

api:
	uv run uvicorn app.asgi:app --reload --port 8000

worker:
	uv run celery -A app.celery_app.celery_app worker -Q extract,vad,asr,translate,tts,qc,stitch -l info

lint:
	uv run ruff check app tests

fmt:
	uv run ruff check --fix app tests && uv run ruff format app tests

typecheck:
	uv run mypy app

test:
	uv run pytest -n auto

ci: lint typecheck test

# WSL note: the repo lives on /mnt/f (Windows drive). uv can't hardlink there,
# and a stale .venv/lib64 symlink breaks every run — hence UV_LINK_MODE=copy
# and the lib64 cleanup guard below.
preflight:
	rm -f .venv/lib64 .venv/lib 2>/dev/null || true
